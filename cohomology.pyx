...
cdef long[:,:] multiindex_to_index(long [:,:] action_image, target_basis_dic, long component):
    cdef size_t num_rows = action_image.shape[0]
    cdef long[:,:] lookup_map = np.stack(
        [action_image, np.ones(num_rows, dtype=np.int_) * component], axis=0
    )
    cdef long[:] target_inds = np.zeros(num_rows, dtype=np.int_)
    cdef size_t j
    for j in range(num_rows):
        target_inds[j] = target_basis_dic[tuple(lookup_map[j])]
    return np.stack([target_inds,action_image[:,-2], action_image[:,-1]],axis=1)
...
def coker_reduce(target_module, coker, long[:,:] action_image, mu0, mu1, component=0):
    ...
    # cdef long[:,:] coker_mat
    cdef Matrix_integer_dense coker_mat
    cdef size_t num_coker_rows
    if mu0 in coker:
        # If target vector space is zero, return empty matrix
        coker_mat = coker[mu0]
        num_coker_rows = coker_mat.nrows()
        if num_coker_rows==0:
            return np.array([])
        new_images = np.zeros((action_image.shape[0]*coker_mat.ncols(),3),dtype=np.int_)
        current_row = 0
    ...
    if mu1 in coker:
        ...
        # If target vector space is zero, return empty matrix
        if coker_mat.nrows()==0:
            return np.array([])
        new_image_coker = np.zeros((new_action_image.shape[0]*coker_mat.ncols(),3),dtype=np.int_)
        current_row = 0
...
def coker_reduce_alt(target_module, coker, long[:,:] action_image, mu0, mu1, component=0):
    """Projects source and target of an action in the coker quotient coker(f), f:M->N.
    Returns action in the basis of the cokernel.
    `source_module` is the module M
    `target_module` is the module N
    ...
    # then `num_cols` denotes this number k.
    cdef size_t num_cols = action_image.shape[1]-2
    cdef size_t num_action_image_rows = action_image.shape[0]
    mat_space = MatrixSpace(ZZ,num_action_image_rows,num_cols+2)
    cdef Matrix_integer_dense action_image_mat = Matrix_integer_dense(mat_space,action_image)
    ...
    cdef size_t num_action_rows = action_image.shape[0]
    # cdef long[:,:] action_image_target = np.zeros((num_action_rows, 3),dtype=np.int_)
    cdef Matrix_integer_dense action_image_target = Matrix_integer_dense(MatrixSpace(ZZ, num_action_rows, 3, False, None))
    # cdef long[:,:] new_action_image
    cdef Matrix_integer_dense new_action_image
    cdef long i,j,k
    cdef Integer c, coeff, target, source
    # cdef long i, j, k, c, coeff, target, source
    cdef long[:] row
    # for i,row in enumerate(action_image):
    for i in range(num_action_rows):
        # row = action_image[i]
        c = Integer(target_basis_dic[tuple(list(action_image[i, :num_cols])+[component])])
        action_image_target.set_unsafe(i,0,c)
        action_image_target.set_unsafe(i,1,action_image_mat[i, num_cols])
        action_image_target.set_unsafe(i,2,action_image_mat[i, num_cols+1])
    ...
    cdef size_t current_row
    # cdef long[:,:] new_images
    cdef Matrix_integer_dense new_images
    # cdef long[:,:] coker_mat
    cdef Matrix_integer_dense coker_mat
    cdef size_t num_coker_rows
    if mu0 in coker:
        ...
        if num_coker_rows==0:
            return np.array([])
        # new_images = np.zeros((action_image.shape[0]*coker_mat.ncols(),3),dtype=np.int_)
        new_images = Matrix_integer_dense(MatrixSpace(ZZ, num_action_image_rows*coker_mat.ncols(),3, False, None))
        current_row = 0
        for k in range(num_action_rows):
            target = action_image_target[k,0]
            source = action_image_target[k,1]
    ...
    # If it's not in the dictionary, no reduction is necessary.
    # cdef long[:,:] new_image_coker
    cdef Matrix_integer_dense new_image_coker
    cdef size_t nrows_new_action_image = new_action_image.nrows()
    ...
    if mu1 in coker:
        ...
        if coker_mat.nrows()==0:
            return np.array([])
        # new_image_coker = np.zeros((new_action_image.shape[0]*coker_mat.ncols(),3),dtype=np.int_)
        new_image_coker = Matrix_integer_dense(MatrixSpace(ZZ, new_action_image.nrows()*coker_mat.ncols(),3, False, None))
        current_row = 0
    ...
    # If the final matrix is empty, instead we just return an empty array to avoid errors.
    if current_row>0:
        return sort_merge(new_image_coker[:current_row].numpy())
    else:
        return np.array([])
